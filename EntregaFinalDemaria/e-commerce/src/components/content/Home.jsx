        ...
        getItems().then(productos => {
            const card = productos.map(producto => 
                <div className="card cardHome" key={producto[0]} style={{width: '18rem'}}>
                    <img className="card-img-top cardImg" src={producto[1].img} alt={producto[1].title} />
                    <div className="card-body cardText">
                        <h4 className="card-title">{producto[1].title}</h4>
                        <h5 className="card-text">${producto[1].price}</h5>
                    </div>
                    <button className="btn btn-primary cardBtn"><Link className='nav-link' to={"/item/" + producto[0]}>Ver Detalle</Link></button>   
                </div>)
            setProductos(card)
        })
    }, [])
